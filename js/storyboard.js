...
export function initStoryboard(section, gsap, lenis, ScrollTrigger) {
  ...
  const exLayer  = section.querySelector('.af__exhibits');   // parallax rides this one container
  ...
  // ── ember twinkle + ignition angle — precomputed once, no per-frame alloc ──
  // twinkle: a static per-particle brightness factor plus a slow individual pulse,
  // so the field reads as living embers rather than a uniform swarm. Costs one
  // sine-LUT lookup per particle per frame (the LUT is already hot).
  const pTw = new Float32Array(N), pTwSpd = new Float32Array(N), pTwPh = new Float32Array(N);
  for (let i = 0; i < N; i++) {
    pTw[i]    = 0.72 + Math.random() * 0.46;
    pTwSpd[i] = 0.6  + Math.random() * 1.1;
    pTwPh[i]  = Math.random() * 6.283;
  }
  // per-particle angle on the rest ring — drives the one-time ignition sweep only.
  const pAng = new Float32Array(N);
  for (let i = 0; i < N; i++) pAng[i] = Math.atan2(restT[i * 2 + 1], restT[i * 2]);

  // ── background dust — a sparse, far layer that never morphs. DISCRETE dim
  // points (not a soft wash): individual points are noise, so they add depth and
  // atmosphere WITHOUT the 8-bit gradient banding a low-alpha wash would cause on
  // the flat #141414 bg. Each drifts on its own slow sine and rides a shallow
  // (deep-plane) parallax. Drawn as one extra GPU call behind the glyph; count
  // scales with the particle budget so weak devices stay light.
  const D = Math.max(160, Math.round(N * 0.24));
  const DUST_COL = [206, 220, 245];   // cool near-white motes — deep-layer atmosphere, independent of the hue
  const dust = {
    x: new Float32Array(D), y: new Float32Array(D), a: new Float32Array(D), sz: new Float32Array(D),
    phx: new Float32Array(D), phy: new Float32Array(D), spx: new Float32Array(D), spy: new Float32Array(D),
    plx: new Float32Array(D),
  };
  for (let i = 0; i < D; i++) {
    dust.x[i]   = Math.random();            // 0..1 across the stage width
    dust.y[i]   = Math.random();            // 0..1 across the stage height
    dust.sz[i]  = 0.6 + Math.random() * 1.2;
    dust.a[i]   = 0.10 + Math.random() * 0.16;
    dust.phx[i] = Math.random() * 6.283; dust.phy[i] = Math.random() * 6.283;
    dust.spx[i] = 0.05 + Math.random() * 0.12; dust.spy[i] = 0.05 + Math.random() * 0.12;
    dust.plx[i] = 0.10 + Math.random() * 0.30;
  }
  const dustArr = new Float32Array(D * 4);   // reused every frame — no per-frame alloc
  ...
  // envelope clocks (perf.now stamps): formation bloom fires on every activation /
  // return to rest; the ignition sweep fires once, on first entrance.
  let activateAt = -1e9, igniteAt = -1e9;
  // smoothed "activeness" (0 at rest → 1 while a tool is formed) — drives the
  // sustained glow so a settled glyph stays alive instead of cooling to inert.
  let actLvl = 0;
  function setActive(idx) {
    ...
    active = idx;
    activateAt = performance.now();          // fire the formation bloom
  ...
  function setDefault() {
    if (active === -1) return;
    active = -1;
    activateAt = performance.now();          // bloom on the way back to rest too
  ...
  function draw() {
    ...
    const tm = nowMs * 0.001;
    // formation bloom — a brief brightness surge after each activation / return to
    // rest (setActive/setDefault stamp activateAt), so a morph lands with a visible
    // "click" and colour switches flash hot instead of easing through a muddy RGB
    // midpoint. One scalar for the whole field; no per-particle cost to compute.
    let bloom = 0;
    if (!reduced) {
      const bt = (nowMs - activateAt) * 0.001;
      if (bt >= 0 && bt < 0.7) bloom = fsin((bt / 0.7) * 3.14159) * 0.5;   // 0 → peak → 0
    }
    // one-time ignition sweep — a bright band travels once around the rest ring on
    // first entrance (runEntrance stamps igniteAt), then never runs again.
    let sweepA = -99; const sweepW = 0.9;
    if (!reduced) {
      const it = (nowMs - igniteAt) * 0.001;
      if (it >= 0 && it < 0.95) sweepA = -3.14159 + (it / 0.95) * 7.5;     // sweeps past 2π so all angles are hit
    }
    // sustained active glow — once a glyph has formed it stays energised (a soft
    // brightness lift + a slow, coherent "breath") rather than cooling to the inert
    // rest brightness. actLvl eases in on activation and out on return to rest, so
    // this is one global scalar per frame; the idle ring never picks it up.
    actLvl += ((active !== -1 ? 1 : 0) - actLvl) * 0.08;
    let activeGlow = 0;
    if (!reduced && actLvl > 0.002) activeGlow = actLvl * (0.34 + 0.12 * fsin(tm * 1.7));
    ...
    // ── background dust — a far, non-morphing layer drawn behind the glyph. One
    // extra draw call of ~D dim discrete points; gives the ring a volume to sit in
    // instead of a flat wall. It lives inside draw(), so it inherits every existing
    // throttle (scroll half-rate, FPS-guard pause, IntersectionObserver gate).
    {
      const mv = reduced ? 0 : 1;   // reduced-motion: static dust, no drift/parallax
      for (let i = 0; i < D; i++) {
        const o = i * 4;
        dustArr[o]     = dust.x[i] * w + (fsin(tm * dust.spx[i] + dust.phx[i]) * 6 + px * 10 * dust.plx[i]) * mv;
        dustArr[o + 1] = dust.y[i] * h + (fcos(tm * dust.spy[i] + dust.phy[i]) * 6 + py * 8  * dust.plx[i]) * mv;
        dustArr[o + 2] = (1.8 + dust.sz[i] * 1.3) * scale;
        dustArr[o + 3] = dust.a[i];
      }
      if (useGL) {
        glR.drawDust(dustArr, D, DUST_COL, curRdpr);       // cool near-white motes, low alpha
      } else {
        ctx.globalCompositeOperation = 'lighter';
        for (let i = 0; i < D; i++) {
          const o = i * 4, dd = dustArr[o + 2];
          ctx.globalAlpha = dustArr[o + 3];
          ctx.drawImage(sp, dustArr[o] - dd / 2, dustArr[o + 1] - dd / 2, dd, dd);
        }
      }
    }
    ...
    for (let i = 0; i < drawN; i++) {
      ...
      const px0 = px * 16 * (pPlx[i] - 0.9), py0 = py * 12 * (pPlx[i] - 0.9); // depth parallax

      let sx = cx + (curS[x] + dxj) * scale + px0;
      let sy = cy + (curS[y] + dyj) * scale + py0;

      // ── brightness / size composition (multiplicative, capped at the end) ──
      // ember twinkle: static per-particle factor × a slow individual pulse.
      let a = pAlpha[i] * pTw[i] * (1 + 0.14 * fsin(tm * pTwSpd[i] + pTwPh[i]));
      let dot = baseDot * pSize[i];
      // velocity glow (A): particles in motion (morphing) burn hotter and a touch
      // larger, cooling as they land. Uses the morph delta already computed above,
      // so it's ~free; rest-state rotation is sub-threshold, so the idle ring stays calm.
      const spd2 = mdx * mdx + mdy * mdy;
      if (spd2 > 0.25) { const vb = spd2 * 0.02 > 0.9 ? 0.9 : spd2 * 0.02; a += a * vb; dot += dot * vb * 0.5; }
      // formation bloom (F): brief global surge after each activation / return to rest.
      if (bloom > 0) a += a * bloom;
      // sustained active glow: keeps the formed glyph alive (lift + slow breath).
      if (activeGlow > 0) { a += a * activeGlow; dot += dot * activeGlow * 0.3; }
      // one-time ignition sweep (F): a bright band circles the rest ring on entrance.
      if (sweepA > -90) {
        let da = pAng[i] - sweepA;
        if (da > 3.1416) da -= 6.2832; else if (da < -3.1416) da += 6.2832;
        if (da < 0) da = -da;
        if (da < sweepW) a += a * (1 - da / sweepW) * 1.3;
      }
      ...
      if (a > 1.6) a = 1.6;                 // cap so stacked boosts can't blow to white

      if (useGL) {
        glR.setParticle(i, sx, sy, dot, a);
      } else {
  ...
  function runEntrance() {
    igniteAt = performance.now();            // one-time ring-ignition sweep starts here
  ...
  let exLastX = 1e9, exLastY = 1e9;   // last exhibit-parallax offset written (skip redundant writes)
  function loop() {
    ...
    // Exhibit counter-parallax (C): the visible panel pair rides a gentler parallax
    // than the field (same direction, smaller magnitude) so they read as a nearer
    // plane — and, as a bonus, this only ever moves them AWAY from the centre glyph,
    // so clearance never shrinks. One GPU-only transform on a single container,
    // skipped while scrolling and when the pointer is effectively still.
    if (exLayer && !scrolling) {
      if (section.classList.contains('is-exhibiting')) {
        const exx = px * 10, exy = py * 7;
        if (Math.abs(exx - exLastX) > 0.05 || Math.abs(exy - exLastY) > 0.05) {
          exLayer.style.transform = 'translate3d(' + exx.toFixed(2) + 'px,' + exy.toFixed(2) + 'px,0)';
          exLastX = exx; exLastY = exy;
        }
      } else if (exLastX !== 1e9) {
        exLayer.style.transform = ''; exLastX = 1e9; exLastY = 1e9;
      }
    }
  }
